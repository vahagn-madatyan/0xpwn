...
class TestNmapIntegration:
    ...
    async def test_nmap_localhost_scan(self, docker_sandbox: DockerSandbox) -> None:
        ...
        Port 80 will likely be closed — that's fine.  The important thing
        is that nmap runs, produces XML, and the parser returns structured
        output with a hosts list.
        """
        ...
        # There should be at least one host entry for localhost
...
class TestCleanup:
    ...
    async def test_cleanup_orphans(self) -> None:
        ...
        # Leave container running — cleanup_orphans should find it
